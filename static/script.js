    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (result.path && result.path.length > 0) {
                ...
                pathDisplay.style.color = 'var(--text-color)';
                
                const pathText = result.path.map((city, index) => {
                    if (index === 0) return `Source (${city})`;
                    if (index === result.path.length - 1) return `Destination (${city})`;
                    return `City ${city}`;
                }).join(' → ');
                
                pathDisplay.textContent = `Path:     ${pathText}`;
                resultText.parentNode.appendChild(pathDisplay);
            }
